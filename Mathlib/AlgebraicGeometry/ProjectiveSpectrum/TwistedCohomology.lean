...
theorem monomialCoeffShift_self (a : LaurentExp n d)
  ...
  simp only [monomialCoeffShift, monomialElemShift, HomogeneousLocalizedModule.Away.val_mk,
...
theorem monomialCoeffShift_smulMonomialElemShift
  ...
  simp only [monomialCoeffShift, smulMonomialElemShift, HomogeneousLocalizedModule.Away.val_mk,
    LocalizedModule.liftOn_mk, monomialCoeffShiftFun]
  rw [coeff_shift_coordProdFinsupp_eq_of_pow_eq (R := R) S (a.numFinsupp S)
    (Exists.choose_spec (p := fun m => _ ^ m = _ ^ _) _)]
...
theorem algebraicComplex_shift_acyclic_pos (d : ℤ) (hd : 0 ≤ d) (p : ℕ) :
  ...
  intro b hb
  show (monomialCoeffShiftHom b T hb) _ = 0
  rw [map_sub]
  have h_lhs : (monomialCoeffShiftHom b T hb) (algebraicδ n R 𝓜 p (fun ⟨S, hS⟩ =>
      B_fin.sum fun a =>
        if h : a.negSupport ⊆ S then
          smulMonomialElemShift (R := R) d hd (ga a ⟨S, hS, h⟩) a S h
        else 0) ⟨T, hT⟩) =
  ...
  have h_rhs : (monomialCoeffShiftHom b T hb) (f ⟨T, hT⟩) =
  ...
  ext ⟨S, hS, hb'⟩
  simp only [componentHomShift, AddMonoidHom.coe_mk, ZeroHom.coe_mk]
  show (monomialCoeffShiftHom b S hb') _ = _
  rw [map_sum]
  simp_rw [show ∀ (a : LaurentExp n d),
    (monomialCoeffShiftHom b S hb')
      (if h : a.negSupport ⊆ S then
        smulMonomialElemShift (R := R) d hd (ga a ⟨S, hS, h⟩) a S h
      else 0) =
    if h : a.negSupport ⊆ S then
      monomialCoeffShift b S hb'
        (smulMonomialElemShift (R := R) d hd (ga a ⟨S, hS, h⟩) a S h)
