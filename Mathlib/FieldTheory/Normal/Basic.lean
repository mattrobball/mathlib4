...
instance normal_iSup {ι : Type*} (t : ι → IntermediateField F K) [h : ∀ i, Normal F (t i)] :
  ...
  have that₁ := this.map (inclusion hE).toRingHom
  have that₂ := this.map (inclusion hE).toRingHom
  -- have := this.map (inclusion hE  : E →+* (⨆ i, t i : IntermediateField F K))
  -- rw [Polynomial.map_map, AlgHom.comp_algebraMap_of_tower] at this
  -- simp only [Polynomial.map_map, AlgHom.comp_algebraMap_of_tower] at this
  -- set_option pp.all true in
  show_term simp only [AlgHom.toRingHom_eq_coe, Polynomial.map_map, AlgHom.comp_algebraMap_of_tower] at that₁
  -- set_option pp.all true in
  show_term rw [AlgHom.toRingHom_eq_coe, Polynomial.map_map, AlgHom.comp_algebraMap_of_tower] at that₂
  simp only at that₂
  -- have : that = this := by with_reducible rfl
  exact that₂
  -- simpa only [Polynomial.map_map] using (this.map (inclusion hE).toRingHom)
  -- exact this
  -- rw [AlgHom.toRingHom_eq_coe, Polynomial.map_map, AlgHom.comp_algebraMap_of_tower] at this
  -- exact this
  -- show_term rwa [Polynomial.map_map]
  -- show_term rwa [Polynomial.map_map] at this
  -- show_term simpa only [AlgHom.toRingHom_eq_coe, Polynomial.map_map, AlgHom.comp_algebraMap_of_tower] using this.map (inclusion hE).toRingHom

#check AlgHom.toRingHom_eq_coe
