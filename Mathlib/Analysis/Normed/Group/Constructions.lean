...
instance seminormedCommGroup
    [CommGroup E] [SeminormedCommGroup E] [CommGroup F] [SeminormedCommGroup F] :
...
lemma Pi.enorm_single [DecidableEq ι] [∀ i, AddCommGroup (G i)] [∀ i, NormedAddCommGroup (G i)]
    {i : ι} (y : G i) : ‖Pi.single i y‖ₑ = ‖y‖ₑ := by simp [enorm, Pi.nnnorm_single]

theorem Pi.norm_single [DecidableEq ι] [∀ i, AddCommGroup (G i)] [∀ i, NormedAddCommGroup (G i)]
    {i : ι} (y : G i) : ‖Pi.single i y‖ = ‖y‖ :=
...
lemma nnnorm_unop [AddGroup E] [SeminormedAddGroup E] (a : Eᵐᵒᵖ) :
    ‖MulOpposite.unop a‖₊ = ‖a‖₊ := rfl
