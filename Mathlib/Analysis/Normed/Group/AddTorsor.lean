...
class NormedAddTorsor (V : outParam Type*) (P : Type*) [AddCommGroup V] [SeminormedAddCommGroup V]
  [PseudoMetricSpace P] extends AddTorsor V P where
  dist_eq_norm' : ∀ x y : P, dist x y = ‖(x -ᵥ y : V)‖

/-- Shortcut instance to help typeclass inference out. -/
instance (priority := 100) NormedAddTorsor.toAddTorsor'
    {V P : Type*} [AddCommGroup V] [NormedAddCommGroup V]
    [MetricSpace P] [NormedAddTorsor V P] : AddTorsor V P :=
  NormedAddTorsor.toAddTorsor

variable {α V P W Q : Type*} [AddCommGroup V] [SeminormedAddCommGroup V]
variable [PseudoMetricSpace P] [NormedAddTorsor V P]
variable [AddCommGroup W] [NormedAddCommGroup W] [MetricSpace Q] [NormedAddTorsor W Q]
...
def pseudoMetricSpaceOfNormedAddCommGroupOfAddTorsor (V P : Type*)
    [AddCommGroup V] [SeminormedAddCommGroup V] [AddTorsor V P] : PseudoMetricSpace P where
  dist x y := ‖(x -ᵥ y : V)‖
  dist_self x := by simp
...
is not an instance because it depends on `V` to define a `MetricSpace P`. -/
def metricSpaceOfNormedAddCommGroupOfAddTorsor (V P : Type*) [AddCommGroup V] [NormedAddCommGroup V]
